...
# Dempster-Shafer a fusion
def dempster_shafer_fusion(mass_functions):
        ...
        for set1, mass1 in m1.items():
            for set2, mass2 in m2.items():
                ...
                if not intersect:
                    ...
                    m[intersect] = m.get(intersect, 0) + mass1 * mass2
        
        if k == 1:
            # Complete conflict, cannot be combined, may need to handle this situation
            return {}
    ...
    for i in range(1, len(mass_functions)):
        result = combine_two(result, mass_functions[i])
    return result
...
class BayesianLinear(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        nn.init.normal_(self.weight_rho, mean=0, std=0.1)
        nn.init.zeros_(self.bias_mu)
        nn.init.normal_(self.bias_rho, mean=0, std=0.1)
...
class EvidentialLayer(nn.Module):
    ...
    def forward(self, x):
        # The output of the linear layer is interpreted as evidence
...
class UQModule:
    ...
    @staticmethod
    def expected_entropy(list_of_probabilities):
        # E[H(p)] = sum_over_samples(H(p_sample)) / num_samples
        entropies = [UQModule.predictive_entropy(p) for p in list_of_probabilities]
        return torch.mean(torch.stack(entropies), dim=0)

    @staticmethod
    def mutual_information(probabilities, list_of_probabilities):
        # I(y, w) = H(E[p]) - E[H(p)]
        mean_probs = torch.mean(torch.stack(list_of_probabilities), dim=0)
        entropy_of_mean = UQModule.predictive_entropy(mean_probs)
        mean_of_entropies = UQModule.expected_entropy(list_of_probabilities)
